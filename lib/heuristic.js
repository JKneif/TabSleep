...
(function (global) {
  ...
  const DISCARD_THRESHOLD = 20;
  const TAB_COUNT_MULTIPLIER_THRESHOLD = 20;
  const TAB_COUNT_MULTIPLIER = 1.5;
  const INACTIVE_MINUTES = 2;
  const AUDIBLE_MINUTES = 0;
  // Counters in the content script are reported per 60s window.
  // v0.2: thresholds halved from conservative defaults — aggressive mode.
  const LONG_TASKS_THRESHOLD = 2;
  const WS_RECONNECTS_THRESHOLD = 1;
  const LAYOUT_SHIFTS_THRESHOLD = 4;
